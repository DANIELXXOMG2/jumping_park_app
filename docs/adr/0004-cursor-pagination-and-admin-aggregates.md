...
- `#540` — `sdd/comprehensive-product-audit-and-roadmap/design` chose cursor-first admin APIs with aggregate read models because Firestore bills skipped documents and full-scan dashboards do not scale well on budget.
- `#555` — `Completed Phase 3 admin data-plane with manual smoke blocker` records that cursor-ready list contracts, the `admin_metrics` read model, and immutable admin audit writes were applied together as the live admin data-plane direction.
