...
- `#540` — `sdd/comprehensive-product-audit-and-roadmap/design` chose “assets/session cache -> local consent queue -> deterministic sync ledger” over a full client-side Firestore rewrite.
- `#556` — `Implemented offline consent queue rollout` records the production detail that the original dedupe key must survive retry/replay, otherwise duplicate consents can be created after a mid-flight failure.
