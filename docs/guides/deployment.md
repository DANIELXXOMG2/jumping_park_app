...
4. Deploy the app on Vercel with `bun install --frozen-lockfile` and `bun run build`.
...
Set these values in `.env.example`. The table shows the current defaults from the file:
...
### Rollout flags (hardening policy)

These flags are read by `src/lib/hardeningPolicy.ts` (see `HARDENING_FLAG_ENV_KEY` mapping at lines 11-19). When not set, each flag defaults to the value listed in `HARDENING_FLAG_DEFAULT_ENABLED` (lines 21-29). Set them explicitly in Vercel so deploy behavior is reviewable.

| Variable | Default | Safe starting value | Why |
| --- | --- | --- | --- |
| `OTP_HARDENING_ENABLED` | `true` | `true` | Keep OTP throttling and lockouts active |
| `EXPORT_BOUNDS_ENFORCED` | `true` | `true` | Keep wide export protections active |
| `PUBLIC_SEO_ENABLED` | `true` | `false` until public validation passes | Runtime defaults are secure, but the public route should only open after `docs/runbooks/seo-ai-seo-validation-checklist.md` passes on a real deployment |
| `CURSOR_PAGINATION_ENABLED` | `false` | `false` | Enable later only after Firebase index parity is confirmed |
| `ADMIN_AGGREGATES_ENABLED` | `false` | `false` | Enable later only after read-model/IaC validation |
| `OFFLINE_QUEUE_ENABLED` | `false` | `false` | Keep disabled until the replay drill is intentionally run |
| `CSP_REPORT_ONLY_ENABLED` | `false` | `false` | Enable only when actively observing the canary CSP |

Note: `NEXT_PUBLIC_OFFLINE_QUEUE_ENABLED` is read by `hardeningPolicy.ts` line 133 and should stay aligned with `OFFLINE_QUEUE_ENABLED`. Keep both disabled unless you are intentionally enabling the offline queue.
...
- Use `CONTRIBUTING.md` for the current quality-gate map before future releases.
