          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex flex-col items-center gap-0.5 px-1.5 py-1.5 rounded-lg transition-colors min-h-0",
                  isActive ? "text-primary" : "text-foreground/60"
                )}
              >
                <item.icon className="w-5 h-5" />
                <span className="text-[9px] font-medium truncate max-w-[50px]">{item.label}</span>
              </Link>
            );
          })}
        </div>
