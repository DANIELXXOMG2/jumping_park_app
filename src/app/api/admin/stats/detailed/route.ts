...
export async function GET(request: NextRequest) {
  try {
    ...
    const daysToShow = period === "today" ? 1 : period === "week" ? 7 : period === "month" ? 30 : period === "year" ? 12 : 30;
    ...
    if (period === "year") {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        dailyData.push({
          date: monthStart.toLocaleDateString("es-CO", { month: "short", year: "2-digit" }),
          consents,
          users,
          minors,
        });
      ...
      for (let i = daysToShow - 1; i >= 0; i--) {
        ...
        dailyData.push({
          date: dayStart.toLocaleDateString("es-CO", { day: "2-digit", month: "short" }),
    ...
    let totalMinors = 0;
    const minorsCountSnap = await db.collection("users")
      .select("minors")
    ...
    return NextResponse.json({
      ...
      topDays,
      averages: {
        consentsPerDay: daysToShow > 0 ? Math.round((consentsSnap.size / daysToShow) * 10) / 10 : 0,
        minorsPerConsent: consentsSnap.size > 0 ? Math.round((minorsInPeriod / consentsSnap.size) * 10) / 10 : 0,
      },
    });
  } catch (error) {
