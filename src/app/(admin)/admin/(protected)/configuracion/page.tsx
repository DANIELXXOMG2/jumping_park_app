...
function ClauseEditor({ clause, index, onChange, onDelete }: ClauseEditorProps) {
              ...
              <div className="flex flex-wrap gap-3">
                <label className="flex items-center gap-2 text-sm">
                  <input
                    ...
                    onChange={(e) =>
                      onChange(index, { ...clause, highlight: e.target.checked })
                    }
                ...
                {clause.highlight && (
                  <>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        onChange(index, { ...clause, highlightLabel: e.target.value })
                      }
                ...
                {rule.highlight && (
                  <>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        onChange(index, { ...rule, highlightLabel: e.target.value })
                      }
                      className="px-2 py-1 rounded border border-border bg-background text-sm w-32"
                      placeholder="Etiqueta"
                    />
...
export default function ConfiguracionPage() {
  const [content, setContent] = useState<ConsentContentStructure>(DEFAULT_CONSENT_CONTENT);
  const [isLoading, setIsLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [saveStatus, setSaveStatus] = useState<SaveStatus>({ type: "idle", message: "" });
  const [activeSection, setActiveSection] = useState<"consent" | "rules">("consent");

  // Cargar contenido actual
  const fetchContent = useCallback(async () => {
    setIsLoading(true);
    try {
      const result = await adminGet<{ data: ConsentContentStructure | null }>("/api/admin/settings/consent");
      if (result.data) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveStatus({ type: "success", message: "Cambios guardados exitosamente" });
  ...
  const handleReset = () => {
    if (confirm("¿Estás seguro de restaurar los valores por defecto? Se perderán todos los cambios.")) {
      setContent(DEFAULT_CONSENT_CONTENT);
    }
  };
  ...
  const handleAddClause = () => {
    ...
    setContent({
      ...
      consent: {
        ...content.consent,
        clauses: [...content.consent.clauses, { id: newId, text: "Nueva cláusula..." }],
      },
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-foreground/60 text-sm mt-1">
            Edita el contenido del consentimiento informado y las reglas del parque
          </p>
        </div>
      ...
      {activeSection === "consent" && (
        <div className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Textos del Consentimiento</CardTitle>
            </CardHeader>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setContent({
                      ...content,
                      consent: { ...content.consent, introduction: e.target.value },
                    })
                  }
                  rows={3}
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setContent({
                      ...content,
                      consent: { ...content.consent, closingStatement: e.target.value },
                    })
                  }
                  rows={3}
                  className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 resize-y"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setContent({
                      ...content,
                      rules: { ...content.rules, closingMessage: e.target.value },
                    })
                  }
                  className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
                />
      ...
      <div className="bg-blue-500/10 text-blue-600 dark:text-blue-400 p-4 rounded-lg text-sm">
        ...
        <p className="text-foreground/70">
          Usa <code className="bg-blue-500/20 px-1 rounded">{"{COMPANY_NAME}"}</code> en el texto
          de las cláusulas para que se reemplace automáticamente por el nombre de la empresa.
        </p>
      </div>
    </div>
  );
