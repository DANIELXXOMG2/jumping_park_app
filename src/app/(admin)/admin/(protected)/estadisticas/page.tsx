        ...
        <div className="space-y-3">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="space-y-1">
              ...
              <div className="h-5 bg-surface-muted rounded animate-pulse" style={{ width: `${Math.random() * 50 + 30}%` }} />
            </div>
          ))}
        </div>
      </CardContent>
...
export default function EstadisticasPage() {
  ...
  const fetchStats = useCallback(async (forceFresh = false) => {
    ...
    try {
      setIsLoading(true);
      const result = await adminGet<StatsData>(`/api/admin/stats/detailed?period=${period}`);
  ...
  const maxChartValue = data?.chartData
    ? Math.max(...data.chartData.map((d) => Math.max(d.consents, d.users, d.minors)), 1)
    : 1;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Card className="relative overflow-hidden">
              ...
              <CardContent className="pt-6">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm text-foreground/60 font-medium">Consentimientos</p>
                    <p className="text-3xl font-bold text-foreground mt-1">
                      {data.kpis.consents.value.toLocaleString()}
                    </p>
                    {data.kpis.consents.change !== undefined && period !== "all" && (
                      <div className="flex items-center gap-1 mt-2">
                        ...
                        <span
                          className={cn(
                            "text-sm font-medium",
                            data.kpis.consents.change >= 0 ? "text-green-500" : "text-red-500"
                          )}
                        >
                          {data.kpis.consents.change > 0 ? "+" : ""}
            ...
            <Card className="relative overflow-hidden">
              ...
              <CardContent className="pt-6">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm text-foreground/60 font-medium">Nuevos Usuarios</p>
                    <p className="text-3xl font-bold text-foreground mt-1">
                      {data.kpis.users.value.toLocaleString()}
                    </p>
                    {data.kpis.users.change !== undefined && period !== "all" && (
                      <div className="flex items-center gap-1 mt-2">
                        ...
                        <span
                          className={cn(
                            "text-sm font-medium",
                            data.kpis.users.change >= 0 ? "text-green-500" : "text-red-500"
                          )}
                        >
                          {data.kpis.users.change > 0 ? "+" : ""}
            ...
            <Card className="relative overflow-hidden">
              ...
              <CardContent className="pt-6">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm text-foreground/60 font-medium">Menores Registrados</p>
                    <p className="text-3xl font-bold text-foreground mt-1">
                      {data.kpis.minors.value.toLocaleString()}
                    </p>
                    {data.kpis.minors.change !== undefined && period !== "all" && (
                      <div className="flex items-center gap-1 mt-2">
                        ...
                        <span
                          className={cn(
                            "text-sm font-medium",
                            data.kpis.minors.change >= 0 ? "text-green-500" : "text-red-500"
                          )}
                        >
                          {data.kpis.minors.change > 0 ? "+" : ""}
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-4 mb-4 text-xs">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-foreground/60">Consentimientos</span>
                    </div>
                    <div className="flex items-center gap-1">
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-foreground/70">Total Usuarios</span>
                      <span className="text-lg font-bold text-foreground">
                        {data.totals.users.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-foreground/70">Total Consentimientos</span>
                      <span className="text-lg font-bold text-foreground">
                        {data.totals.consents.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-foreground/70">Total Menores</span>
                      <span className="text-lg font-bold text-foreground">
            ...
            <CardContent className="py-4">
              <div className="flex flex-wrap items-center justify-between gap-4 text-sm">
                <div className="flex items-center gap-2 text-foreground/60">
                  ...
                  <span>
                    Período: {new Date(data.dateRange.start).toLocaleDateString("es-CO")} -{" "}
                    {new Date(data.dateRange.end).toLocaleDateString("es-CO")}
                  </span>
                </div>
                {period !== "all" && data.kpis.consents.previousValue !== undefined && (
                  <div className="text-foreground/50 text-xs">
                    Período anterior: {data.kpis.consents.previousValue} consentimientos,{" "}
                    {data.kpis.users.previousValue} usuarios, {data.kpis.minors.previousValue} menores
                  </div>
                )}
              </div>
            </CardContent>
