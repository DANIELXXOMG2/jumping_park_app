...
function ExitoContent() {
	...
	const nombre =
		searchParams.get("nombre") || (language === "es" ? "Visitante" : "Visitor");
	...
	return (
		<main
			className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-background p-4 sm:p-6"
		...
		>
			...
			<div className="relative mb-6 sm:mb-8">
				...
				<div className="relative bg-gradient-to-br from-primary via-emerald-400 to-primary rounded-full p-6 sm:p-8 shadow-[0_0_60px_rgba(46,204,113,0.5)]">
					<CheckCircle2
						size={60}
						className="text-zinc-900 sm:w-20 sm:h-20"
						strokeWidth={1.5}
					/>
			...
			<div className="text-center max-w-md relative z-10">
				...
				<div className="group relative bg-gradient-to-br from-white/10 via-white/5 to-white/10 dark:from-zinc-800/80 dark:via-zinc-900/60 dark:to-zinc-800/80 border-2 border-primary/30 rounded-2xl sm:rounded-3xl p-4 sm:p-6 mb-6 sm:mb-8 overflow-hidden">
					{/* Shimmer effect */}
					<span
						className="absolute inset-0 bg-gradient-to-r from-transparent via-primary/10 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"
						aria-hidden="true"
					/>
					...
					<div className="flex items-center justify-center gap-2">
						<span className="text-4xl sm:text-5xl font-bold bg-gradient-to-r from-primary via-emerald-400 to-primary bg-clip-text text-transparent">
							{offlineMode ? "Pendiente" : `#${consecutivo}`}
						</span>
					</div>
					<p className="text-foreground/50 text-xs sm:text-sm mt-2 sm:mt-3 flex items-center justify-center gap-1.5">
						...
						{offlineMode
							? "Guardado localmente. Se enviara cuando vuelva la conexion."
							: t("exito.saved")}
					</p>
				</div>
				...
				<div className="relative bg-gradient-to-r from-primary/10 via-emerald-500/10 to-primary/10 border border-primary/30 rounded-xl sm:rounded-2xl p-3 sm:p-4 mb-6 sm:mb-8 overflow-hidden">
					{/* Partículas */}
					<span
						className="absolute top-2 left-3 w-1.5 h-1.5 rounded-full bg-primary/40 animate-pulse"
						aria-hidden="true"
					/>
					<span
						className="absolute bottom-2 right-4 w-2 h-2 rounded-full bg-emerald-400/30 animate-pulse delay-300"
						aria-hidden="true"
					/>
				...
				<output
					aria-live="polite"
					className="mt-3 flex items-center justify-center gap-2 text-xs text-foreground/40 sm:mt-4 sm:text-sm"
				>
