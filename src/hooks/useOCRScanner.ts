...
const PATTERNS = {
	...
	FECHA_NACIMIENTO:
		/(?:F\.?\s*Nac\.?|Nacimiento|Fecha\s*Nac\.?)[\s.:]*(\d{1,2}[-/]\d{1,2}[-/]\d{2,4})/gi,
	FECHA_ISO: /(\d{4}[-/]\d{2}[-/]\d{2})/g,
	FECHA_DMY: /(\d{1,2}[-/]\d{1,2}[-/]\d{2,4})/g,
	// Nombres (busca después de palabras clave)
	NOMBRE:
		/(?:Nombres?|NOMBRES?)[\s.:]+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+(?:\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)*)/g,
	APELLIDO:
		/(?:Apellidos?|APELLIDOS?)[\s.:]+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+(?:\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)*)/g,
};
	...
	return new Promise((resolve, reject) => {
		...
		img.onload = () => {
			...
			for (let i = 0; i < data.length; i += 4) {
				...
				const threshold = 140;
				const finalValue =
					enhanced > threshold ? 255 : enhanced < 80 ? 0 : enhanced;
