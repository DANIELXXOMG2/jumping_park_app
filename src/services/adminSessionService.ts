...
const logger = createLogger("AdminSessionService");
...
type AdminIdTokenVerifier = (idToken: string) => Promise<DecodedAdminIdToken>;
...
export async function exchangeAdminSessionFromIdToken(
	...
	try {
		...
		if (
			...
			logger.error("Missing ADMIN_JWT_SECRET");
			throw new ApiError(
		...
		if (isFirebaseAdminConfigurationError(error)) {
			logger.error("Firebase Admin misconfiguration", error);
			throw new ApiError(
