...
export function buildAdminSessionCookieValue(payload: AdminSessionPayload): string {
	assertSessionSecret()
...
async function verifyBearerToken(
	...
	try {
		...
		if (error instanceof Error && error.message.includes('expired')) {
			return getUnauthorizedResponse('Token expirado. Inicia sesion nuevamente.')
		}
...
function validateRequiredRole(
	...
	if (requiredRole && requiredRole !== payload.role && payload.role !== 'admin') {
		return getForbiddenResponse(
			`Se requiere rol '${requiredRole}' para esta accion`,
		)
	}
